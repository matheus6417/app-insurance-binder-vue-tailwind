<template>
  <div class="p-3 px-5 absolute w-full bottom-0 left-0 text-white flex justify-between"
  :style="{
        backgroundColor:
    ...
    <div class="inline-flex">
      <button class="bg-white hover:bg-gray-200 py-1 px-1 rounded mr-1"
       :disable="!canGoTo('prev')"
        @click="canGoTo('prev') ?  $store.commit('changeCurrentQuestion', $store.state.currentQuestion - 1) : void 0"
        :class="canGoTo('prev') ? 'bg-white' : 'bg-gray-400 cursor-not-allowed'"
        >
        <BIcon :icon="'chevron-up'" :color="'black_default'"></BIcon>
      </button>
      <button class="hover:bg-gray-200 py-1 px-1 rounded"
      :class="canGoTo('next') ? 'bg-white' : 'bg-gray-400 cursor-not-allowed'"
      :disable="!canGoTo('next')"
      @click="canGoTo('next') ? $store.commit('changeCurrentQuestion', $store.state.currentQuestion + 1) : void 0">
